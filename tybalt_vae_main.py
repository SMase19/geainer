...
def learning_loop(name, xp, dataset, args, model, optimizer):
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        if epoch % 4 == 0 and isinstance(model, ae.VariationalAutoEncoder):
...
def main():
    ...
    models = OrderedDict({
        "simple": ae.SimpleAutoEncoder(n_input, n_latent),
        "sparse": ae.SparseAutoEncoder(n_input, n_latent),
        "deep": ae.DeepAutoEncoder(n_input),
        "convolutional": ae.ConvolutionalAutoEncoder(n_input),
        "variational": ae.VariationalAutoEncoder(784, n_latent, 500)
    })
